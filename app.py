...
# ─────────────────────────────────────────────────────────────
# TENSORFLOW  (lazy import so cold-start errors are readable)
# ─────────────────────────────────────────────────────────────
try:
...
#
#  How to get it:
#  1. Open Google Drive → right-click best_model.h5 → Share
...
.login-title {
    font-family:'Space Mono',monospace; font-size:1.8rem; color:#e94560;
    text-align:center; letter-spacing:4px; font-weight:700; margin-bottom:2px;
}
.login-sub { color:#888; text-align:center; font-size:0.82rem; margin-bottom:28px; letter-spacing:1px; }
.login-divider { border:none; border-top:1px solid #2a2a2a; margin:0 0 22px 0; }
.main-header {
    background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);
    border-radius:16px; padding:32px 40px; margin-bottom:28px;
    border:1px solid #e94560; text-align:center;
}
.main-header h1 { font-family:'Space Mono',monospace; font-size:2.4rem; color:#e94560; margin:0; letter-spacing:2px; }
.main-header p  { color:#a0aec0; font-size:1rem; margin-top:8px; }
.card { background:#161616; border:1px solid #2a2a2a; border-radius:12px; padding:20px 24px; margin-bottom:18px; }
.card-title {
    font-family:'Space Mono',monospace; font-size:0.85rem; color:#e94560;
    letter-spacing:1.5px; text-transform:uppercase; margin-bottom:14px;
    border-bottom:1px solid #2a2a2a; padding-bottom:10px;
}
.instrument-badge {
    display:inline-block; background:linear-gradient(90deg,#e94560,#c0392b);
    color:white; padding:6px 14px; border-radius:20px; font-weight:700; font-size:0.9rem; margin:4px;
}
.instrument-badge-dim {
    display:inline-block; background:#2a2a2a; color:#666;
    padding:6px 14px; border-radius:20px; font-size:0.85rem; margin:4px;
}
.conf-row { display:flex; align-items:center; margin-bottom:10px; gap:10px; }
.conf-label { width:160px; font-size:0.9rem; color:#e0e0e0; flex-shrink:0; }
.conf-bar-outer { flex:1; background:#2a2a2a; border-radius:6px; height:14px; overflow:hidden; }
.conf-bar-inner-high { height:100%; border-radius:6px; background:linear-gradient(90deg,#e94560,#ff6b6b); }
.conf-bar-inner-low  { height:100%; border-radius:6px; background:#3a3a3a; }
.conf-val { width:52px; text-align:right; font-family:'Space Mono',monospace; font-size:0.82rem; color:#aaa; }
.step-label {
    background:#e94560; color:white; border-radius:50%; width:28px; height:28px;
    display:inline-flex; align-items:center; justify-content:center;
    font-weight:700; font-size:0.9rem; margin-right:10px; font-family:'Space Mono',monospace;
}
.stFileUploader > div { border:2px dashed #e94560 !important; border-radius:12px !important; background:#111 !important; }
.stDownloadButton > button {
    background:linear-gradient(90deg,#e94560,#c0392b) !important; color:white !important;
    border:none !important; border-radius:8px !important; font-weight:700 !important; padding:10px 20px !important;
}
section[data-testid="stSidebar"] { background:#111 !important; border-right:1px solid #2a2a2a; }
.stAlert { border-radius:10px !important; }
.metric-box { background:#1a1a1a; border:1px solid #2a2a2a; border-radius:10px; padding:16px; text-align:center; }
.metric-val { font-family:'Space Mono',monospace; font-size:1.6rem; color:#e94560; font-weight:700; }
.metric-lbl { font-size:0.8rem; color:#888; text-transform:uppercase; letter-spacing:1px; }
.logout-btn > button {
    background:transparent !important; border:1px solid #e94560 !important; color:#e94560 !important;
    border-radius:8px !important; font-weight:600 !important; width:100% !important; margin-top:6px;
}
.logout-btn > button:hover { background:#e94560 !important; color:white !important; }
</style>
...
@st.cache_resource(show_spinner="⬇️ Downloading model from Google Drive...")
def load_model():
    if not TF_OK:
        st.error("TensorFlow is not installed. Check your requirements.txt.")
        return None
    if not GDOWN_OK:
        st.error("gdown is not installed. Check your requirements.txt.")
        return None

    # Download from Google Drive if not already cached
    if not os.path.exists(MODEL_PATH):
        if GDRIVE_FILE_ID == "PASTE_YOUR_FILE_ID_HERE":
            st.error("❌ Please set your GDRIVE_FILE_ID in app.py")
            return None
...
if not TF_OK:
    st.error("❌ TensorFlow not found. Add `tensorflow` to your requirements.txt and redeploy.")
    st.stop()
if not GDOWN_OK:
    st.error("❌ gdown not found. Add `gdown` to your requirements.txt and redeploy.")
    st.stop()
if GDRIVE_FILE_ID == "PASTE_YOUR_FILE_ID_HERE":
    st.warning("⚠️ Set your `GDRIVE_FILE_ID` in app.py, then redeploy.")
    st.stop()
...
with col_s1:
    ...
    <div class="metric-box">
        <div class="metric-val">{'✅' if model_ready else '⬇️'}</div>
        <div class="metric-lbl">{'Model Ready' if model_ready else 'Will Download'}</div>
    </div>""", unsafe_allow_html=True)
with col_s2:
    st.markdown(f"""
    <div class="metric-box">
...
if st.button("🎯 Detect Instruments", use_container_width=True):
    ...
    if agg is None:
        st.error("Model could not be loaded. Check your GDRIVE_FILE_ID and Drive sharing settings.")
        st.stop()
    ...
    if detected:
        ...
        st.warning("No instrument confidently detected above threshold. Try lowering the threshold in the sidebar.")
